document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        if (!yValue || isNaN(yValue) || yValue <= -5 || yValue >= 3) {
        ...
        try {
            ...
            if (data.error) {
                formError.textContent = `Ошибка сервера: ${data.error}`;
                return;
            }
            ...
            formError.textContent = `Ошибка при отправке запроса: ${error.message}`;
